...
pub unsafe fn register_all(conn: duckdb_connection) {
    register_build_aggregate(conn, "postgis_spatial_index_build", "spatial");
    register_query_tf(
        conn,
        "postgis_spatial_index_query_envelope",
        QueryKind::Envelope,
    );
    register_query_tf(conn, "postgis_spatial_index_query_knn", QueryKind::Knn);
    register_query_tf(
        conn,
        "postgis_spatial_index_within_distance",
...
// === extension: postgis ===
// index `spatial` type_id=0 caps={"knn":true,"update_after_build":false,"within_distance":true,"within_distance_wkb":true}
// index `gist` type_id=0 caps={"knn":true,"update_after_build":false,"within_distance":true,"within_distance_wkb":true}
// index `rtree` type_id=0 caps={"knn":true,"update_after_build":false,"within_distance":true,"within_distance_wkb":true}
